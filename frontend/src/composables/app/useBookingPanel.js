...
export const panelWidth  = ref(510);
...
export const closeModal    = () => {
  ...
  modalInit.value = null;
  editBooking.value = null;
};
export const openEditModal = (b) => {
  editBooking.value = b;
  modalInit.value = null;
...
// "새 예약" 버튼은 항상 새 예약 모드로 전환/유지한다.
export const openNewBooking = () => {
  // 이미 새 예약 모드로 열려 있으면 닫기(토글)
  if (showModal.value && !editBooking.value) {
    closeModal();
    return;
  }

  // 수정 모드에서 누르면 새 예약 모드로 전환
  editBooking.value = null;
  modalInit.value = null;
  showModal.value ? modalKey.value++ : (showModal.value = true);
};
...
const calcPopupStyle = (rect, popupW = 296, popupH = 400) => {
  const gap = 8;
  ...
  // 상하: 카드 상단 기준, 무조건 viewport 안에 들어오도록 위로 끌어올림
  const maxTop = vh - popupH - gap;
  const top = Math.max(gap, Math.min(rect.top, maxTop));

  return {
    top: top + 'px',
    left: Math.max(gap, left) + 'px',
    maxHeight: (vh - top - gap) + 'px',
  };
};
...
export const openDetail = (b, event) => {
  ...
  if (event) {
    const rect = event.currentTarget.getBoundingClientRect();
    popoverStyle.value = calcPopupStyle(rect, 560, 520);
  } else {
    popoverStyle.value = {
      top: '50%',
      left: '50%',
