...
import { sortBy, targetDate, searchQuery, isMobile } from './useCalendar';
...
export const canCancel = (b) =>
  isAdmin || b.userId === currentUser.id;
...
// 수직 타임라인 (일간 뷰)
export const HOUR_H   = computed(() => (isMobile.value ? 60 : 80));
...
export const calcPosV = (b, totalLanes, color) => {
  ...
  const rawHeight = (endH - startH) * HOUR_H.value - 3;
  return {
    position: 'absolute',
    top:      `${(startH - 6) * HOUR_H.value}px`,
    height:   `${Math.max(rawHeight, MIN_BLOCK_H)}px`,
...
export const nowLineTop = computed(() =>
  `${(liveNow.value.hour() + liveNow.value.minute() / 60 - 6) * HOUR_H.value}px`
);
...
export const scrollDayView = () => {
  ...
  dayViewRef.value.scrollTop = Math.max(0, (h - 6) * HOUR_H.value - 40);
};
