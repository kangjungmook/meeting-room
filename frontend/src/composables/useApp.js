...
watch(viewMode, (newVal, oldVal) => {
  ...
  // 주간 뷰 진입 시 오늘 날짜로 리셋
  if (newVal === 'week') targetDate.value = dayjs();
  fetchBookings();
...
import {
  ...
  slideDir as _slideDir, hours, isMobile, sidebarCollapsed, showDrawer, showDateSheet,
...
import {
  ...
  startResize, onResizeMove, onResizeEnd, closeModal, openEditModal, openQuickModal, openNewBooking,
...
import {
  ...
  notifPermission, pushEnabled, initFcm, requestNotifPermission,
  enablePush, disablePush, connectSse, disconnectSse,
...
export function useApp() {
  return {
    ...
    // UI 상태
    isMobile, sidebarCollapsed, showDrawer, showDateSheet,
    // 시계
    liveNow, liveTime, liveDate,
    // 토스트
    toasts, toastDuration, addToast,
    // 예약 패널
    showModal, panelWidth, resizing, modalKey, modalInit, editBooking,
    closeModal, openEditModal, openQuickModal, openNewBooking,
    ...
    // FCM
    notifPermission, pushEnabled, initFcm, requestNotifPermission, enablePush, disablePush,
    // SSE
