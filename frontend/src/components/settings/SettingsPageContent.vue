        ...
        <div class="flex flex-1 min-h-0" :class="isMobile ? 'flex-col' : ''">
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar">
            ...
            <div v-if="activeTab === 'notifications'" class="flex flex-col min-h-full">
              ...
              <div class="px-6 pt-6 flex flex-col gap-7" style="padding-bottom: calc(env(safe-area-inset-bottom, 0px) + 96px)">
                <div>
                ...
                <!-- 푸시 알림 수신 설정 -->
                <div>
                  <p class="text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-3">푸시 알림 수신 설정</p>
                  <div class="border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden bg-white dark:bg-gray-900">

                    <!-- 마스터 ON/OFF 토글 -->
                    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                      <div>
                        <p class="text-[13.5px] font-semibold text-gray-700 dark:text-gray-200">이 기기 푸시 알림</p>
                        <p class="text-[12px] text-gray-400 dark:text-gray-500 mt-0.5">
                          {{ notifPermission !== 'granted' ? '켜면 브라우저 알림 권한을 요청합니다' : draftPushEnabled ? '이 기기에서 알림을 수신 중입니다' : '이 기기의 알림이 꺼져 있습니다' }}
                        </p>
                      </div>
                      <button type="button"
                              @click="notifPermission !== 'granted' ? requestNotifPermission() : draftPushEnabled = !draftPushEnabled"
                              :class="['relative flex-shrink-0 w-11 h-6 rounded-full transition-colors duration-200',
                                       notifPermission !== 'granted' ? 'opacity-40 bg-gray-200 dark:bg-gray-700' :
                                       draftPushEnabled ? 'bg-blue-500' : 'bg-gray-200 dark:bg-gray-700']">
                        <span :class="['absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-200',
                                       draftPushEnabled && notifPermission === 'granted' ? 'left-[22px]' : 'left-0.5']"></span>
                      </button>
                    </div>

                    <!-- 알림 종류별 토글 (푸시가 켜진 경우만 활성) -->
                    <div v-for="(item, i) in pushItems" :key="item.key"
                         ...
                         :class="[i < pushItems.length - 1 ? 'border-b border-gray-100 dark:border-gray-800' : '',
                                  (!draftPushEnabled || notifPermission !== 'granted') ? 'opacity-40' : '']">
                      <div>
                      ...
                      <button type="button"
                              :disabled="!draftPushEnabled || notifPermission !== 'granted'"
                              @click="userNotifPrefs[item.key] = !userNotifPrefs[item.key]"
                              :class="['relative flex-shrink-0 w-11 h-6 rounded-full transition-colors duration-200',
...
const { showDrawer, currentUser, userNotifPrefs, saveNotifPrefs, darkMode, toggleDarkMode,
        ...
        notifPermission, pushEnabled, requestNotifPermission, enablePush, disablePush } = useApp();

const isDirty = ref(false);
const savedPrefs = ref(null);

// 저장 전 임시 푸시 상태 (실제 pushEnabled 와 분리)
const draftPushEnabled = ref(pushEnabled.value);
const savedPushEnabled = ref(pushEnabled.value);
...
const checkDirty = () => {
  ...
  isDirty.value = JSON.stringify(cur) !== JSON.stringify(saved)
               || draftPushEnabled.value !== savedPushEnabled.value;
};

onMounted(() => {
  connectSse();
  draftPushEnabled.value  = pushEnabled.value;
  savedPushEnabled.value  = pushEnabled.value;
...
watch(userNotifPrefs, checkDirty, { deep: true });
watch(draftPushEnabled, checkDirty);
...
const handleSave = async () => {
  // 푸시 ON/OFF 상태가 바뀐 경우                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        에만 서버에 반영
  if (draftPushEnabled.value !== pushEnabled.value) {
    if (draftPushEnabled.value) await enablePush();
    else await disablePush();
    savedPushEnabled.value = draftPushEnabled.value;
  }
