 <template>
  ...
  >
    ...
    <div v-if="sidebarCollapsed" class="h-full flex flex-col items-center py-4 gap-3">
      ...
      <!-- 새 예약 버튼 -->
      <button @click="openNewBooking" title="새 예약"
      ...
      <div class="flex flex-col items-center gap-1 mt-1">
        <button @click="router.push('/main')" title="전체 예약 현황"
        ...
        <button @click="router.push('/my-bookings')" title="내 회의"
                :class="['w-8 h-8 flex items-center justify-center rounded-lg transition-colors',
        ...
        <button @click="router.push('/settings')" title="설정"
                :class="['w-8 h-8 flex items-center justify-center rounded-lg transition-colors',
                  route.path === '/settings'
                    ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
                    : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500']">
